...
class database extends schema{
    ...
    // 
    //Retrieve the user roles from this database. 
    //A role is an entity that has 2 columns named; email and user.
    public get_roles():Array<{key:string, value:string}>{
        // 
        //Define the roles to be extracted from this dbase.
        ...
        const user_entities = Object
            ...
            //
            //Select those entities that have ...
            .filter(Entity=>
                //
                // ... a column named user ...
                Entity.columns.user !== undefined 
                //
                // ... and another column called email. 
                && Entity.columns.email !== undefined
            );
            //
            //
        return user_entities.map(entity => {
            //
            //Ensure that title has the same value as the id if it is missing.
            const title = entity.title === undefined ? entity.name : entity.title;
            //
            //Compile and return the desired key value pairs.
